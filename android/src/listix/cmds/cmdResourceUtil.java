...
#gastonaDoc#
   ...
   <help>
      ...
      //
      // Gastona has at least one micro-tool which is sqlite (sqlite3.exe or sqlite3 for linux) and
...
<!               , EXTENSION  , (mp3)
   ...
   <syntaxHeader>
      synIndx, importance, desc
         ...
         3   ,    2      , //Return the fullpath of the executable program related with the micro tool 'microToolName'. If the micro tool is not yet installed then this will be done first
   ...
   */
   public int execute (listix that, Eva commands, int indxComm)
   {
      ...
      if (cmd.meantConstantString (subCmd, new String [] { "COPYURL" } ))
      {
         ...
         if (!urlUtil.copyUrl (resourceName, toFileName, null))
         {
