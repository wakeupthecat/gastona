...
public class javaLoad
{
   ...
   */
   ...
   /**
      unzip method
   */
   public static void unZipResourceZip (String zipFile, String basePath)
   {
      boolean ok = true;
      byte [] buff = new byte[8192];

      int resId = androidSysUtil.getResourceId ("raw/" + zipFile);
      if (resId == 0)
      {
         log.err ("unZipResourceZip", "resource name [" + "raw/" + zipFile + "] not found! (resource names given without extension)");
         return;
      }

      log.dbg (2, "unZipResourceZip", "unzip [" + "raw/" + zipFile + "] on [" + basePath + "]");
      try
      {
         // Archiv öffnen und mit Stream verbinden
         ZipInputStream in = new ZipInputStream(androidSysUtil.getResources().openRawResource(resId));
         ...
         while ((entry = in.getNextEntry()) != null)
         {
            ...
            if (entry.isDirectory ())
            {
               ...
               log.dbg (2, "unZipResourceZip", "create directory [" + targetFileName + "]");

               // set date
               File fi = new File (targetFileName);
               if (fi == null)
               {
                  log.err ("unZipResourceZip", "Directory [" + targetFileName + "] results null ?!");
                  continue;
               }
               fi.mkdirs ();
            ...
            {
               log.err ("unZipResourceZip", "cannot create directories for target file [" + targetFileName + "]");
               return ;
            }
            ...
            {
               log.err ("unZipResourceZip", "target file [" + targetFileName + "] could not be opened for write");
               return ;
            }

            log.dbg (2, "unZipResourceZip", "unzipping [" + entry.getName() + "] on [" + basePath + "]");

            File fi = new File (targetFileName);
            if (fi == null)
            {
                log.err ("unZipResourceZip", "File [" + targetFileName + "] results null ?!");
                continue;
            }
            File fipa = fi.getParentFile ();
            fipa.mkdirs ();
      ...
      {
         ok = false;
         log.severe ("unZipResourceZip", "unzip [" + zipFile + "] " + e);
      }
      log.dbg (4, "unZipResourceZip", "unzip " + ((ok) ? "well done.": "ended with errors!"));
   }
}
