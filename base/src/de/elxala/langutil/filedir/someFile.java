...
public class someFile
{
   ...
   // NOTE: This class is a special one regard to logger because logServer uses someFiles to protocol
   //       all clients, therefore the special logStatic handling and the special "someFile (logger specialLogger)" constructor
   ...
   //(o) techNotes/logServer/Depencency with someFile
   //
   // special constructor with the only propose
   // that someFile can be used by logServer (it has to write logs)
   // without causing recurrency since someFile also uses logger !
   //
   // specifically logServer has to create a someFile(null) so this
   // file will be used to write the log for the logServer but
   // the own logs of the someFile will not be processed. In other
   // words any "own log" will cause a new someFile creation etc.
   //
   public someFile (logger specialLogger)
   {
      if (voidlogger == null)
   ...
   private void initStatic ()
   {
      if (logStatic == null)
      {
         logStatic = new logger (null, "de.elxala.langutil.filedir.someFile", null);
      }
   }
   ...
   */
   public boolean fopen (String nom, String modo, boolean allowMemFile)
   {
      ...
      boolean forRead = modo.startsWith ("r");
      ...
      try
      {
         if (modo.equals ("r") || modo.equals ("rt"))
         {
      ...
      catch (java.io.FileNotFoundException e)
      {
         bRet = false;
         if (forRead)
            // not a bug, just first attempt failed. someFile accepts files in own jar or other classpath location
