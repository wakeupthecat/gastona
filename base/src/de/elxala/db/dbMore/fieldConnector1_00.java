...
public class fieldConnector1
{
   ...
   */
   ...
   /*
      ...
      with which it is possible to build an SQL (e.g. method printSQL ())
   ...
   public boolean resolveConnections (Eva evaColumns, boolean hasTitlesInRow0, String tableBase, String shortAliasForBase)
   {
      ...
      for (int cc = deepTableIndx0; cc < evaColumns.rows (); cc ++)
      {
         ...
         if (resultListFROM.size () == 0)
         {
            ...
            //
            resultListFROM.add (bUsingFirstAlias ? (firstTable + " AS " + shortAliasForBase): firstTable);
         }

         // ====START CODE 28.02.2010 11:48
         //String firstTa = (String) currConnArray.get(0);
         //String currResiduo = shortAliasForBase ? "": firstTa;
         //if (!resultListFROM.contains (firstTa))
         //   resultListFROM.add (firstTa);
         // ====END OLD CODE 28.02.2010 11:48
         ...
         while (currConnArray.size () > 2)
         {
            ...
            if (indx >= 0)
            {
               ...
               String aliasTargetTable = currResiduo;
               ...
               do
               {
                  ...
                  condition += previaTable + "." + tableConnections.getValue (indx, dbMore.CONN_INDX_SOURCEKEY) + // srcField
                              " == " +
                              aliasTargetTable + "." + tableConnections.getValue (indx, dbMore.CONN_INDX_TARGETKEY); // targetField
                  indx ++;
               ...
               String elemeFrom = targetTable + " AS " + aliasTargetTable;
         ...
         if (groupFunc.length () == 0)
         {
            ...
            resultStringGroupBy += columnAlias;
         }
         else
         {
         ...
         //
         colName = nameQuali + " AS " + columnAlias;
...
29.11.2009 01:30
  ...
  necesita la tabla de conexiones pero no el schema pues construye simplemente los
  strings, si no funcionan es otra cosa!
