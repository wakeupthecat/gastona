...
<!         9   , ...              ,            , //Further additional directory
   ...
   <options>
      ...
      1      , LAUNCH BROWSER , "0 / 1", 0 or 1, //Launches a browser pointing to the port on starting the server. If a port is provided this option is 0 per default and 1 otherwise
      1      , UPLOAD MAX SIZE, "(bytes)", 10000000, //Max size in bytes for a file to upload (default less that 10 MB)
      1      , UPLOAD SOCKET TIMEOUT, "(ms)", 40000, //Max socket timeout in milliseconds, time between partial reads, default is 40 s
      ...
      6      , LAUNCH BROWSER , "0 / 1", 0 or 1, //Launches a browser pointing to the port on starting the server. If a port is provided this option is 0 per default and 1 otherwise
   ...
   */
   public int execute (listix that, Eva commandEva, int indxComm)
   {
      ...
      if (optStart || optOneGet)
      {
         ...
         // LAUNCH BROWSER (default 0 but if portNr==-1 default 1)
         boolean isLaunchBrowser = "1".equals (cmd.takeOptionString(new String [] { "LAUNCHBROWSER", "LAUNCH", "BROWSER" }, (portNr == -1 ? "1": "0")));
         ...
         if (! mose.isServerValid ())
         {
            cmd.getLog().err ("MicoHttpServer", "server " + serverName + " couldn't be started!" );
            return 1;
         }
         ...
         // options after opening the server ...
         // UPLOAD MAX SIZE actually only for openStart since for OneGet make no much sense
         mose.uploadMaximumSizeBytes = stdlib.atoi (cmd.takeOptionString(new String [] { "SETUPLOADMAXSIZE", "UPLOADMAXSIZE" }, "" + micoHttpServer.DEFAULT_MAXSIZE_BYTES_UPLOADING));

         // UPLOAD SOCKET TIMEOUT Actually only for openStart since for OneGet make no much sense
         mose.uploadSocketTimeout = stdlib.atoi (cmd.takeOptionString (new String [] { "SETUPLOADSOCKETTIMEOUT", "UPLOADSOCKETTIMEOUT", "UPLOADTIMEOUT" }, "" + micoHttpServer.DEFAULT_SOCKET_TIMEOUT_WHEN_UPLOADING));
