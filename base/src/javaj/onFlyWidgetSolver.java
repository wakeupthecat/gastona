...
//      else log.dbg (2, "getNativeWidget", "found getWidgetByName \"" + theName + "\"");
   ...
   private static boolean startsWithCaseLess (String str1, String strStart)
   {
      if (str1.length () < strStart.length ()) return false;
      String str = str1.substring(0, strStart.length ());
      return str.equalsIgnoreCase (strStart);
   }
...
//         case 'd': theComp = new zSlider       (fullName); break;
         ...
         case 't': theComp = new xyScrollPane   (new zTable (fullName)); break;
         case 's': theComp = new zAsisteTabla  (fullName); break;
         case 'i': theComp = new xyScrollPane   (new zList (fullName)); break;
         case 'h': theComp = new zRadioButtonGroup (fullName); break;
         case 'r':
               if (baseName.startsWith ("rg"))
                    theComp = new zRadioButtonGroup (fullName);
               else theComp = new zRadioButton  (fullName, sRest);
               break;
         case 'k': theComp = new zCheckBox     (fullName, sRest); break;
         ...
         case '2':
               if (startsWithCaseLess (fullName, "2D"))
               {
                  if (startsWithCaseLess (fullName, "2Daaa"))
                     theComp = new zAproGraphic2D (fullName, null);
                  else if (startsWithCaseLess (fullName, "2Dbasic"))
                     theComp = new zBasicGraphic2D (fullName, null);
                  else if (startsWithCaseLess (fullName, "2Dsvg"))
                     theComp = new z2DGraphicScenes (fullName);
                  else theComp = new JButton ( "?" + fullName);
                  //if (startsWithCaseLess (name, "2Dmath")) compos = new z2DMathFunc (co, name);
                  //if (startsWithCaseLess (name, "2Dcampos")) compos = new z2DCampos (co, name);
                  //if (startsWithCaseLess (name, "2Dgambos")) compos = new z2DArgambos (co, name);
               }
               break;
         case 'o': {
                     int tipo = zConsole.STD_BOTH;
                     char secondC = baseName.length () > 1 ? baseName.charAt (1): '.';
                     if (secondC == 's' || secondC == 'o' || secondC == '1')
                        tipo = zConsole.STD_OUTPUT;
                     if (secondC == 'e' || secondC == '2')
                        tipo = zConsole.STD_ERROR;

                     theComp = new xyScrollPane (new zConsole (fullName, tipo));
                  }
                  break;
      ...
      //System.out.println ("miramacho " + baseName + " es un trist " + theComp);
