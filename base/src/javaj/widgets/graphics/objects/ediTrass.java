...
public class ediTrass
{
   ...
   public int trassForm = FORM_POLYGONE; // "path", "oval", ...
   ...
   public boolean isTrassClosed = false;
   public String style = "";
   ...
   public ediTrass (float x, float y, int form)
   {
      posX = x;
      posY = y;
      setTrassForm (form);
   ...
   public void set (ediTrass otro)
   {
      //this = otro;
      trassForm = otro.trassForm;
      posX = otro.posX;
      posY = otro.posY;
      isTrassClosed = otro.isTrassClosed;
      style = otro.style;
      // not deep but shallow copy!
      points = new Vector (otro.points.size ());
      for (int ii = 0; ii < otro.points.size (); ii ++)
      {
         //de.elxala.langutil.uniUtil.printLater ("editTrass::set add in ii " + ii + "[" + points + "]");
         points.add (otro.points.get (ii));
      }
   }

   public void setTrassForm (int tform)
   {
      trassForm = tform;
   }
   ...
   // NOTE: call computeBounds4Point always BEFORE actual insertion of the point in arrEdiTrasss !
   ...
   public boolean setValueAt (int indx, float val)
   {
      if (indx < 0 || indx >= points.size ())
         return false;

      ((float []) points.get(indx))[0] = val;
      return true;
   }
   ...
   public boolean setPointXY (int ptIndx, float xval, float yval)
   {
      return setValueAt (ptIndx+ptIndx+0, xval) &&
             setValueAt (ptIndx+ptIndx+1, yval);
   }

   public float getPointAbsX (int pindx)
   {
      return posX + getPointX (pindx);
   }

   public float getPointAbsY (int pindx)
   {
      return posY + getPointY (pindx);
   }

   public vect3f getPairAt (int pindx)
   {
      return new vect3f (getPointX (pindx), getPointY (pindx));
   }

   public vect3f getPairAbsoluteAt (int pindx)
   {
      return new vect3f (getPointAbsX (pindx), getPointAbsY (pindx));
   }
   ...
   {
      isTrassClosed = isClosed;
   }

   public boolean isClosed ()
   {
      return isTrassClosed;
   }
   ...
   public boolean isForm (int form)
   {
      if (isEmpty ())
         setTrassForm (form);
      return trassForm == form;
   }

   public void addValue (float pt)
   {
      //de.elxala.langutil.uniUtil.printLater ("editTrass::addValue valu " + pt + " points size " + points.size () + " [" + points + "]");
   ...
   public void insertValueAt (int indx, float pt)
   {
      ...
      //de.elxala.langutil.uniUtil.printLater ("editTrass::insertValueAt valu " + pt + " index = " + indx + " points size " + points.size () + " [" + points + "]");
   ...
   public boolean changePairAbs (int indx, float px, float py)
   {
   ...
   public boolean changePairRel (int indx, float dx, float dy)
   {
      if (indx < 0 || indx+indx+1 >= points.size ())
         return false;
   ...
   public void insertPairAt (int pindx, float xval, float yval)
   {
      if (isFormPointArray () && points.size () == 0)
      {
         addValue (0.f);
      ...
      insertValueAt (pindx+pindx+0, xval);
      insertValueAt (pindx+pindx+1, yval); // we have to add 1 anyway!

      // example
      //     points [ 10 20 30 40 ]
      //               0  1  2  3
      //
      // insertPairAt (1, 22, 23)
      //    insertValueAt (1+1,   22)   => [ 10 20 22 30 40 ]
      //    insertValueAt (1+1+1, 23)   => [ 10 20 22 23 30 40 ]
      // oks
   }
   ...
   public boolean isNextAbsPairClose (int pindx)
   {
      return isNextAbsPairClose (pindx, 2.f);
   }

   public boolean isNextAbsPairClose (int pindx, float maxDelta /* = 2.f */)
   {
      if (pindx < 0 || pindx >= points.size ())
         return false;

      return Math.abs (getPointX (pindx) - getPointX (pindx + 1)) < maxDelta &&
             Math.abs (getPointY (pindx) - getPointY (pindx + 1)) < maxDelta
             ;
   }

   public boolean isNextRelPairClose (int pindx)
   {
      return isNextRelPairClose (pindx, 2.f);
   }

   public boolean isNextRelPairClose (int pindx, float maxDelta /* = 2.f */)
   {
      if (pindx < 0 || pindx >= points.size ())
         return false;

      return Math.abs (getPointX (pindx + 1)) < maxDelta &&
             Math.abs (getPointY (pindx + 1)) < maxDelta
             ;
   }

   public void addDuplicated (int pindx)
   {
      if (pindx < 0) return;

      //example points [ 1 2  10 11  20 21 ]
      //
      //    addOrRemoveClonePair (1)  => [ 1 2  10 11  10 11 20 21 ]
      //    addOrRemoveClonePair (1)  => [ 1 2  10 11  20 21 ]
      //
      // or should we consider only addOrRemoveDoublePair (1) ?
      // that is check only the next pair and not the previous one
      //
      insertPairAt (pindx, getPointX (pindx), getPointY (pindx));
   }

   public boolean parseForm (String strTrass)
   {
      int indx = 0;
      int setpoint = 0;
      isTrassClosed = false;
      int MAX_TRAS_TYPE_LEN = 10;
      boolean coordRelative = strTrass.charAt (0) > 'Z';

      // NOTE: in general (Trass in lower case) the points are saved as relative to previous point,
      //       we want to have them in memory relative to initial point, so we have to recalculate them
      //
      String TTLower = strTrass.length() > MAX_TRAS_TYPE_LEN ? strTrass.substring(0, MAX_TRAS_TYPE_LEN): strTrass;
      TTLower = TTLower.toLowerCase ();

      if (TTLower.startsWith ("pol"))
      {
         setTrassForm (FORM_POLYGONE);
         isTrassClosed = TTLower.startsWith ("polz");
         setpoint = 1;
      }
      else
      if (TTLower.startsWith ("rec"))
      {
         setTrassForm (FORM_RECTANGLE);
      }
      else
      if (TTLower.startsWith ("ova") || TTLower.startsWith ("cir") || TTLower.startsWith ("elli"))
      {
         setTrassForm (FORM_OVAL);
      }
      else
      if (TTLower.startsWith ("arc"))
      {
         setTrassForm (FORM_ARC);
         isTrassClosed = TTLower.startsWith ("arcz");
      }
      else
      if (TTLower.startsWith ("cub"))
      {
         setTrassForm (FORM_PATH_CUBIC);
         isTrassClosed = TTLower.startsWith ("cubz");
         setpoint = 3;
      }
      else
      if (TTLower.startsWith ("qua"))
      {
         setTrassForm (FORM_PATH_QUAD);
         isTrassClosed = TTLower.startsWith ("quaz");
         setpoint = 2;
      }
      else
      if (TTLower.startsWith ("jau"))
      {
         setTrassForm (FORM_PATH_AUTOCASTELJAU);
         isTrassClosed = TTLower.startsWith ("jauz");
         setpoint = 1;
      }
      else
      {
      ...
      while (indx <= strTrass.length ()) // NOTE "<=" to process the last value!
      {
         char charat = indx < strTrass.length () ? strTrass.charAt(indx): ' ';
         indx ++;
         if (charat != ' ' && charat != ',')
         {
             //read character
   ...
   //NOTE: for facilitating edition of first point we add an artificial relative 0, 0 point when loading the Trass
   ...
   public String toStringOnlyShape ()
   {
      ...
      String addClosed = isTrassClosed ? "z": "";
      int setpoint = 0;
      switch (trassForm)
      {
