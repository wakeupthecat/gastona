...
public class textAparato extends basicAparato
{
   ...
   public textAparato (MensakaTarget objWidget, textEBS pDataAndControl)
   {
      ...
      Mensaka.suscribe (objWidget, RX_REVERT_DATA, ebs().evaName (ebs().sMSG_REVERT_DATA));
      Mensaka.suscribe (objWidget, RX_REVERT_DATA, widgetEBS.sMSG_FORECAST_DATA_FROM_WIDGET_TO_MODEL);

      Mensaka.suscribe (objWidget, RX_COMMNAD_LOAD,  ebsText().evaName (ebsText().sMSG_LOAD));
      Mensaka.suscribe (objWidget, RX_COMMNAD_SAVE,  ebsText().evaName (ebsText().sMSG_SAVE));
      Mensaka.suscribe (objWidget, RX_COMMNAD_CLEAR, ebsText().evaName (ebsText().sMSG_CLEAR));
      Mensaka.suscribe (objWidget, RX_COMMNAD_GOTO, ebsText().evaName (ebsText().sMSG_GOTOLINE));
   }
   ...
   public void updateControl (JTextComponent co)
   {
      ...
      co.setVisible    (ebsText ().getVisible ());
   }
}
