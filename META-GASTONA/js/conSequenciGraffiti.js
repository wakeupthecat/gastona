/**
 * conSequenciCanvas - convert a simple sequence diagram to a "graffiti"
 * Copyright (c) 2016,2017,2018 Alejandro Xalabarder
 */

// c2d Canvas from canv.getContext('2d');
//
function conSequenciGraffiti (diagData, pFontfamily, pFontsize)
{
   var graff = [];

   pFontfamily = pFontfamily||"Consolas";
   pFontsize = pFontsize||"14";
   ...
   // ----- MEASURING TEXT DIMENSIONS IN PIXELS FOR SVG OR CANVAS (IS THE SAME OR ALMOST THE SAME!)
   //
   // Problem : font-family and font-size should be set before !
   //


   // IT DOES NOT WORK FOR SVG !!! :(

   // var mCanvasMeasure = null;
   // var mSvgTextMeasure = null;
   //
   // function textWidth (txt)
   // {
   //    // do we have svg ? if yes then we are "probably" wanting it ...
   //    //
   ...
   //       if (!mSvgTextMeasure)
   //       {
   //          mSvgTextMeasure = document.createElementNS (SVGNamespace, "text");
   //          mSvgTextMeasure.setAttribute ("font-size", pFontsize);
   //          mSvgTextMeasure.setAttribute ("font-family", pFontfamily);
   //       }
   //
   //       mSvgTextMeasure.textContent = txt;
   //       return mSvgTextMeasure.getBBox ().width;
   //    }
   //    else
   //    {
   //       if (!mCanvasMeasure)
   //       {
   //          mCanvasMeasure = document.createElement ("canvas").getContext('2d');
   //          mCanvasMeasure.font = pFontsize + "px " + pFontfamily; // e.g. "14px Consolas"
   //       }
   //
   //       return mCanvasMeasure.measureText (txt).width;
   //    }
   // }
