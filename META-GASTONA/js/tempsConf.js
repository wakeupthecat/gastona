/**
   tempsConf.js
   Copyright (c) 2019 Alejandro Xalabarder
   ...
   var diagData = {
    ...
    ...alternative schema
                     ...
                     ["time", "agent1", "agent2", "dir", "message" ],
                     [ 0.122,    "A"  ,   "B"   , ">>" , "doThisAction" ],
                     [ 2.234,    "B"  ,   "C"   , ">>" , "theAction" ],
                     [ 3.543,    "B"  ,   "C"   , "<<" , "What action?" ],
                     [ 8.558,    "A"  ,   "B"   , "<<" , "done!" ],


      where
         "time" is optional and can be "stamp", "tstamp"

         "agent1" can be a1 ag1 (if exists then agent2 has to exist as well)

         "dir" for forward direction (agent1 -> agent2) can be any of

               1 > >> -> Tx Rq(request) Req.. Send

               and for the backwards

               2 < << <- Rx Rp(response) Res.. Rec..
*/

function sequenciConf (diagData)
{
   ...
   var iAgent1    = arr[0].indexOf ("agent1");
   var iAgent2    = arr[0].indexOf ("agent2");
   var iDirection = arr[0].indexOf ("dir");
